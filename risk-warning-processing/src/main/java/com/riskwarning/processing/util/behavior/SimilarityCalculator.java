...
public final class SimilarityCalculator {
    ...
    public static double scoreBehaviorToTargetDefault(List<Float> behaviorVec,
                                                      ...
                                                      List<String> behaviorTags,
                                                      List<String> targetTags) {
        return scoreBehaviorToTarget(
                behaviorVec,
                targetVec,
                behaviorTags,
                targetTags,
                DEFAULT_A_BEHAVIOR_TO_X,
                DEFAULT_B_BEHAVIOR_TO_X
        );
    }

    /**
     * 公式：score = a * cos(vecR, vecI)
     *              + b * Jaccard(tagsR, tagsI)
     *              + gamma * Jaccard(industryR, industryI)
     * 用于：法规 -> 指标 的相似度计算
     */
    ...
    public static double scoreRegToIndicatorDefault(List<Float> regVec,
        ...
        return scoreRegToIndicator(
                ...
                regIndustry,
                indIndustry,
                DEFAULT_A_REG_TO_IND,
                DEFAULT_B_REG_TO_IND,
                DEFAULT_GAMMA_REG_TO_IND
        );
    }
    ...
    public static double jaccard(List<String> a, List<String> b) {
        ...
        return uni.isEmpty()
                ? 0.0
                : (double) inter.size() / (double) uni.size();
    }
